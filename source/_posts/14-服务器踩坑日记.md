...
数据库可视化工具如何连接远程数据库？尤其是使用了 Nginx 的情况下？  
如果单单是前者，那么 so easy，工具要输入 IP 和 port，必要时输入 用户名 和 密码 就可以了。  
但如果加上后者呢？  
当我想当然的弄了个二级域名，用它指向远端本地数据库时...哦呼，似乎不行呢，提示了个 `recv(): message len 1347703880 is invalid. Min 16 Max: 48000000` 的错误了呢，这是咋回事呢？  
...
4. 不要将 stream 放入 /etc/nginx/conf.d/xx.conf 中，而应该将其放入 /etc/nginx/nginx.conf 中，因为 conf.d 中的所有配置都是以 http 的方式启动的，且放在该目录下的配置文件，如果包含 stream，那么意味着 http { stream }，会报错(可以打开配置文件自己瞅瞅)
