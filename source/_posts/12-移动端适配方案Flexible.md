...
通过 `rem值 * rem基准值 = 原始的px值` 这个公式，我们可以知道 rem值 是**固定**的，rem基准值 和 原始的px值 成正相关，即基准值越大，原始px值越大，反之亦然。  
rem基准值 的计算是通过 设备分辨率宽度 得到的，不同设备其 rem基准值 不同，也就导致 原始px值 不同，这也是 Flexible 自适应布局的本质原理。Flexible 会自动帮我们计算不同设备的 rem基准值。  

以上内容可知：根据公式，我们只要统一一个设计稿，然后得到 `rem值` 就可以完成自适应。  

## 使用

OK，现在就是根据设计稿来获取 `rem值` 了。  
但问题来了，难道我每次都要手动 `原始的px值 / rem基准值` 计算得到 `rem值`？  
